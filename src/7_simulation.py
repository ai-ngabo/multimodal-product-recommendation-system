...
ENROLLMENT = dict(zip(USERS, [178, 152, 133, 125]))
...
merged = pd.read_csv(FEATURES.parent / "processed" / "merged_dataset.csv") \
    if (FEATURES.parent / "processed" / "merged_dataset.csv").exists() \
    else pd.read_csv(ROOT / "data" / "cleaned" / "merged_dataset.csv")
...
def _novelty_ok(vec, centroids, radii, factor=1.8):
...
def _identify(vec, bundle, cent, rad):
    ...
    ok_nov, _, dist = _novelty_ok(vec, cent, rad)
...
def identify_face(img_or_path):
    ...
    return _identify(vec, face, face_cent, face_rad)


def identify_voice(path):
    vec = audio_features_from_path(path)
    return _identify(vec, voice, voice_cent, voice_rad)


def recommend_product(customer_id):
    """Build a product-model row from the customer's record (or a median fallback)."""
    row = merged[merged.customer_id == customer_id]
    if row.empty:
        base = merged.median(numeric_only=True)
        ctx = {c: base.get(c, 0) for c in product["num_features"]}
        for c in product["cat_features"]:
            ctx[c] = "Unknown"
    else:
        r = row.iloc[0]
        ctx = {c: r[c] for c in product["num_features"]}
        for c in product["cat_features"]:
            ctx[c] = r[c]
    X = pd.DataFrame([ctx])[product["num_features"] + product["cat_features"]]
...
def main():
    ...
    if args.face and args.voice:
        run_transaction(args.face, args.voice,
                        f"file: {Path(args.face).name} + {Path(args.voice).name}")
        return
